            ...
            <div>
                ...
                <div className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)]">
                    <section className="rounded-xl border border-[#292d39] bg-[#11131c]/90 p-6 shadow-[0_18px_55px_rgba(0,0,0,.16)] backdrop-blur-xl sm:p-8">
                        ...
                        <div className="mt-8 border-t border-[#292d39] pt-7">{lead.candidate ? <p className="inline-flex items-center gap-2 text-sm text-[#8ff0bd]"><FiCheckCircle /> Convertido a candidato · {lead.candidate.code}</p> : lead.status === 'DISCARDED' ? <p className="inline-flex items-center gap-2 text-sm text-[#ffb1bd]"><FiTrash2 /> Lead descartado</p> : <button onClick={convert} disabled={form.processing} className="velvet-button">{form.processing ? 'Convirtiendo…' : 'Convertir en candidato →'}</button>}</div>
                    </section>
